...
```{r}
# Load data from tables using SQL queries
...
ui <- fluidPage(
  ...
  # Header with Dark Mode Toggle, Title, and Image
  tags$div(
    ...
    # Dark Mode Checkbox aligned to the right
    tags$div(
  ...
  # JavaScript for Dark Mode Toggle (without shinyjs)
  tags$script(HTML("
  ...
  tabsetPanel(
    tabPanel("Heim",
             h2("Welcome to the Premier League Analysis"),
             p("Explore various statistics and insights from the 2019/20 season."),
             p("Use the navigation tabs to access different analyses, including league tables, team locations, player efficiency, and betting statistics.")
    ),
    tabPanel("Síðustu ár",
      sidebarLayout(
        sidebarPanel(
    ...
    tabPanel("Stöðutafla",
             ...
             fluidRow(
               column(5,  # Adjusted width to allow more space on the right
                      h3("Stöðutafla"),
                      sliderInput("game_week", "Veldu tímabilsviku:", min = 1, max = 38, value = 1),
                      div(class = "highlighted-table", tableOutput("league_table"))
               ),
               column(5, offset = 1,  # Added offset to shift this column further to the right
                      h3("Síðustu leikir"),
                      selectInput("team_select", "Veldu lið", choices = NULL),
                      sliderInput("num_games", "Veldu fjölda leikmanna til að sýna:", min = 1, max = 10, value = 5),
    ...
    # Updated UI Code for Side-by-Side Select Inputs Directly Above Each Plot
tabPanel("Dómararýni",
  h2("Dómararýni"),
...
tabPanel("Liðin",
  ...
  # Action buttons for switching between graph and table view, aligned to the right
  div(
  ...
  # Conditional panel to show graph view
  conditionalPanel(
    condition = "output.view_mode == 'graph'",
    
    # Dropdown to select comparison metric
    ...
    # Display average metric table for the selected two teams
    h3("Average Metric for Each Team"),
    tableOutput("average_metric_table")
  ),
  
  # Conditional panel to show the table view with all teams' averages
  conditionalPanel(
