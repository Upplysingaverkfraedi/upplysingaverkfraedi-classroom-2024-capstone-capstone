...
# Setur upp gögn fyrir content_rating fyrir Leonardo og Kate
leo_movies_count <- leo_movies %>% group_by(content_rating) %>% summarise(count = n()) %>% mutate(percentage = (count / sum(count)) * 100)
kate_movies_count <- kate_movies %>% group_by(content_rating) %>% summarise(count = n()) %>% mutate(percentage = (count / sum(count)) * 100)

# Skilgreina lýsingar fyrir content_rating
rating_labels <- c(
  "R" = "Restricted (17+)", 
  "G" = "General Audience", 
...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Fjöldi mynda eftir Content Rating",
             tabsetPanel(
               tabPanel("Leonardo DiCaprio", plotOutput("leoPlot")),
               tabPanel("Kate Winslet", plotOutput("katePlot"))
             )),
...
server <- function(input, output, session) {
  ...
  # Graf fyrir fjölda mynda með Leonardo DiCaprio eftir content rating
  output$leoPlot <- renderPlot({
    ggplot(leo_movies_count, aes(x = content_rating, y = count, fill = content_rating)) +
      ...
      scale_fill_manual(values = c("R" = "pink", "G" = "purple", "PG-13" = "blue", "NR" = "hotpink", "PG" = "skyblue")) +
      ...
      ylim(0, max(leo_movies_count$count) * 1.2) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
  })
  
  # Graf fyrir fjölda mynda með Kate Winslet eftir content rating
  output$katePlot <- renderPlot({
    ggplot(kate_movies_count, aes(x = content_rating, y = count, fill = content_rating)) +
      ...
      scale_x_discrete(labels = rating_labels) +
      labs(
        title = "Number of Movies with Kate Winslet by Content Rating",
        x = "Content Rating",
        y = "Number of Movies"
      ) +
      ylim(0, max(kate_movies_count$count) * 1.2) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
  })
}
